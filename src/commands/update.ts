...
export default class Update extends Command {
  ...
  static examples = ['<%= config.bin %> update telegram', '<%= config.bin %> update slack --release minor'];
  ...
  static flags = {
    ...
    release: Flags.string({ default: 'patch', description: 'patch | minor | major | version explicita x.y.z' }),
  };

  async run(): Promise<void> {
    const { args, flags } = await this.parse(Update);
    const ids = parseIds(args.apps);
    if (ids.length === 0) {this.error('indica al menos un id: update telegram,slack');}
    for (const id of ids) {
