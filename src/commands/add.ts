...
import { appFile, isValidVersion, PAKE_SCHEMA, type PakeConfig, slugify, writeApp } from '../lib/core';
...
export default class Add extends Command {
  ...
  async run(): Promise<void> {
    ...
    for (const entry of flags.set ?? []) {
      ...
      if (eq === -1) {this.error(`--set ${entry} no tiene formato clave=valor`);}
      config[entry.slice(0, eq)] = parseValue(entry.slice(eq + 1));
    }

    const id = flags.id ?? slugify(flags.name);
    if (fs.existsSync(appFile(id))) {this.error(`ya existe la app "${id}" (${appFile(id)})`);}
