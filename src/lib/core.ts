...
export const PAKE_SCHEMA = 'https://raw.githubusercontent.com/tw93/Pake/main/schema/pake.schema.json';
...
export function readApp(id: string): AppEntry {
  ...
  if (!fs.existsSync(file)) {
    throw new Error(`no existe la app "${id}" en apps/ (usa \`pnpm pake list\` para ver las registradas)`);
  }
...
export function selectApps(ids: string[]): AppEntry[] {
  if (ids.length > 0) {return ids.map(readApp);}
...
export function bumpVersion(version: string, release: string): string {
  ...
  switch (release) {
    ...
    default:
      throw new Error(`release no valido: "${release}" (usa patch, minor, major o una version x.y.z)`);
  }
}
...
export function run(cmd: string, args: string[], options: SpawnSyncOptions = {}): boolean {
  ...
  if (result.error) {throw new Error(`no se pudo ejecutar "${cmd}": ${result.error.message}`);}
